...
// in exit_common/ and is shared with EXIT 2; only path/keybind wiring and any
// address-bearing hooks are per-game here.
...
class Exit1App : public rex::ReXApp {
 public:
  using rex::ReXApp::ReXApp;

  static std::unique_ptr<rex::ui::WindowedApp> Create(
      rex::ui::WindowedAppContext& ctx) {
    return std::unique_ptr<Exit1App>(new Exit1App(ctx, "exit1",
        PPCImageConfig));
  }
  ...
  // On quit, re-write exit1.ini from the live config so an INI from an older
  ...
  //      `update:` device (the very first asset open is `update:\dat\effect\
  //      game_button.gim`). With no `update:` mounted that open returns
  //      0xC000000F and the title thread hangs before the first frame. The base
  //      `dat/` tree lives under the same dump root, and a separate title update
  //      can still override it via --update_data_root, so when the user did not
  //      pass one explicitly we default `update:` to the game root -- a single
  //      --game_data_root then boots the game. (EXIT 2 shares this engine; its
  //      glue inherits the same default.)

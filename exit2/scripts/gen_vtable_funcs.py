#!/usr/bin/env python3
"""
gen_vtable_funcs.py - structural vtable / code-pointer-table scanner (EXIT 2).

Lifted verbatim from the EXIT 1 / SCIV reference and re-pointed to EXIT 2's image
layout. Every safety filter is preserved unchanged; only the per-game constants
(exec range, import-thunk range, scan regions, basefile path, file names) differ.
See the EXIT 1 / SCIV / MSXX history for the full rationale; the short version:
...
each extent). Entries already present in exit2_config.toml / exit2_fixups.toml
are skipped so their explicit size hints / hand-authored boundaries are preserved.

EXIT 2 layout (from default.basefile.exe PE section table, base 0x82000000):
  .rdata 0x82000400-0x8203AB3C   .text 0x82050000-0x822423AC (only exec section)
  .data  0x82250000-0x825B223C (vsz), file-backed first 0x6BE00 (->0x822BBE00)
  import thunks: 166 x 16B at the tail of .text, 0x8224194C-0x822423AC.
Like EXIT 1 (and unlike SCIV) there is no second executable section; .text is the
whole exec range, and the thunk table runs exactly to the end of .text.

Run from the exit2 project dir:
...
# XEX import-thunk table (tail of .text): 0x10-byte stubs (li r3,lib / li r4,ord +
...
OUT = PROJ / "exit2_vtables.toml"

# data regions to scan: (name, lo_addr, hi_addr) -- file-backed bytes only.
# .rdata is fully file-backed; .data raw size is 0x6BE00, so only the first
# 0x6BE00 bytes (up to 0x822BBE00) are real -- beyond that is zero-init BSS.
REGIONS = [
    ("rdata", 0x82000400, 0x8203AB3C),
    ("data",  0x82250000, 0x822BBE00),
]
...
def main():
    ...
    with OUT.open("w", encoding="utf-8") as f:
        ...
        f.write("# it. Entries already in exit2_config.toml / exit2_fixups.toml are\n")

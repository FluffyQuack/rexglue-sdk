#!/usr/bin/env python3
"""
gen_callback_funcs.py - immediate-constructed code-pointer (callback) scanner (EXIT 2).

Lifted verbatim from the EXIT 1 / SCIV reference and re-pointed to EXIT 2's image
layout. Every safety filter (interior-range, switch-base, continuation/funclet,
import-thunk) is preserved unchanged; only the per-game constants differ.
...
each). Entries already present in exit2_config.toml / exit2_fixups.toml /
exit2_vtables.toml are skipped so their hints / prior recovery are preserved and
no duplicate keys are emitted.

EXIT 2 layout: exec range = .text only 0x82050000-0x822423AC (no second exec
section); import thunks 0x8224194C-0x822423AC at the .text tail. See
gen_vtable_funcs.py for the section-table derivation.

Run from the exit2 project dir (after gen_vtable_funcs.py):
...
BASE = 0x82000000
EXLO, EXHI = 0x82050000, 0x822423AC    # executable range (.text only)
...
IMPLO, IMPHI = 0x8224194C, 0x822423AC
# Max insns from a `lis` to its paired `addi`/`ori`. The SCIV reference used 4,
# but EXIT's compiler batches the high halves (several `lis` back-to-back) and
# then the low halves, so a pointer's `addi` can sit well past its `lis` -- on
# EXIT 1 this missed the real registry-init callback (a lis paired with an addi 6
# insns later) at a window of 4 and trapped boot. EXIT 2 is the same engine and
# compiler, so the same widened window applies. The window is only a cap;
# correctness comes from the guards below (same base reg not clobbered/branched-
# over in between, terminator at T-4, not interior/switch/continuation/import-
# thunk), so widening to 16 just lets the natural clobber/branch break bound each
# scan. (EXIT 1 window sweep 4->164, 6->175, 8->177, 12->181, 16->188 new entries
# tapered sharply, confirming genuine scheduled pairs not false positives; codegen
# re-verified clean at 16.)
WINDOW = 16

PROJ = Path(__file__).resolve().parent.parent
BASEFILE = PROJ.parent / "Assets" / "EXIT2" / "default.basefile.exe"
# Function keys already known (config size hints + fixups + vtable recovery).
EXISTING_FILES = [PROJ / "exit2_config.toml", PROJ / "exit2_fixups.toml",
                  PROJ / "exit2_vtables.toml"]
# Files carrying explicit `size`/`end` ranges (used to exclude interior labels).
SIZED_FILES = [PROJ / "exit2_config.toml", PROJ / "exit2_fixups.toml"]
OUT = PROJ / "exit2_callbacks.toml"
...
def main():
    ...
    with OUT.open("w", encoding="utf-8") as f:
        ...
        f.write("# in exit2_config.toml / exit2_fixups.toml / exit2_vtables.toml are omitted.\n")
        f.write("#\n")
